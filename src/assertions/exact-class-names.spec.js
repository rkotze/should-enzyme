    ...
    (renderMethod, methodName) => {
      context(methodName, () => {
        ...
        it('assert should fail as "pizza" is not in the list of CSS classes', () => {
          (() =>
            wrapper.should.have.exactClassNames(
              'special buffalo chicken'
            )).should.throwError(
            /expected '(div|ClassNamesFixture)' to have classNames 'special buffalo chicken' but found 'special buffalo chicken burger'/
          );
        });

        it('assert should fail as "buffalo" is in the list of CSS classes', () => {
          (() =>
            wrapper.should.not.have.exactClassNames(
              'special buffalo chicken burger'
            )).should.throwError(
            /expected '(div|ClassNamesFixture)' not to have classNames 'special buffalo chicken burger' but found 'special buffalo chicken burger'/
          );
        });
      });
    }

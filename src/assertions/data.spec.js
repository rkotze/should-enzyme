    ...
    (renderMethod, methodName) => {
      context(methodName, () => {
        ...
        it('should see useful error message for missing pizza data attribute', () => {
          (() => wrapper.should.have.data('pizza')).should.throwError(
            /expected '(div|DataFixture)' to have attribute 'data-pizza'/
          );
        });

        it('should see useful error message for incorrect expected data attribute value', () => {
          (() => wrapper.should.have.data('tr', 'stuff')).should.throwError(
            /expected '(div|DataFixture)' attribute 'data-tr' to have value 'stuff', instead found 'enzyme'/
          );
        });

        it('should see useful error message for incorrect data attribute', () => {
          (() => wrapper.should.have.data('pizza', 'stuff')).should.throwError(
            /expected '(div|DataFixture)' to have attribute 'data-pizza'/
          );
        });
      });

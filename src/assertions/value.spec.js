    ...
    (renderMethod, methodName) => {
      context(methodName, () => {
        ...
        it('should have "Hands or bunch of bananas?" value from textarea', () => {
          wrapper
            .find('textarea')
            .should.have.value('Hands or bunch of bananas?');
        });
        ...
        it(`assert should fail to see useful error message for select element`, () => {
          (() =>
            wrapper
              .find('select')
              .should.have.value('salad')).should.throwError(
            /expected 'select' to have value 'salad' but found 'pizza'/
          );
        });

        it(`assert should fail to see useful error message for input element`, () => {
          (() =>
            wrapper.find('input').should.have.value('pizza')).should.throwError(
            /expected 'input' to have value 'pizza' but found 'coffee'/
          );
        });
        ...
        it(`assert should fail when trying to get value when no value attribute`, () => {
          (() =>
            wrapper
              .find('#failSelect')
              .should.have.value('pizza')).should.throwError(
            /element 'div' does not have a value/
          );
        });
      });
    }
